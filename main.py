...
async def start(event):
    ...
    if event.sender.id in SEXY:
        await main.send_file(
            event.chat.id,
            file="https://mallucampaign.in/images/img_1695921922.jpg",
...
async def start(event):
    ...
    if event.sender.id in SEXY:
        await main.send_file(
            event.chat.id,
            file="https://telegra.ph/file/4ce070797a15d60c658d9.jpg",
  ...
  if event.sender.id in SEXY:
   if not event.is_group:
       ...
       if not admin and not creator:
           ...
           return
       hmm =  await event.reply("__STARTED NOW ENJOY...__")
       await sleep(18)
       await hmm.delete()
...
print("Join @II_BROKEN_X_ANGEL_II if you facing any kind of issue!!")
